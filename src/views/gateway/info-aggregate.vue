    ...
    data() {
      return {
        table: {
          ...
          loading: false,
          tableChangeList: [],
          columns: pageData.infoAggregate.tableColumns
        },
        pageForm: {
          total: 0,
          index: 1,
    ...
    methods: Object.assign(func(), {
      func_dialog(action, row) {
        switch (action.routeKey) {
          case 'add':
            this.dialog = this.constructDialogData({ dialogData: pageData.infoAggregate.dialog, title: '新增聚合信息', apiKey: action.apiKey })
            this.dialog.form.sort = (this.pageForm.total + 1) * 10
            this.dialog.form.ocelotId = this.$route.query.id
            break
          case 'edit':
            if (row.length !== 1) {
            ...
            this.dialog = this.constructDialogData({ dialogData: pageData.infoAggregate.dialog, title: '修改聚合信息', apiKey: action.apiKey, edit: true, ownData: row[0] })
            this.dialog.form.ocelotId = this.$route.query.id
            break
          // 查看参数
          case 'para':
